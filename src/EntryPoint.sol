...
contract EntryPoint is IEntryPoint {
    ...
    function handleOp(IWallet.UserOperation memory op) external {
        bool validated = IWallet(op.sender).validateOp(op);
        require(validated, "EntryPoint: invalid operation");
        ...
        // is supposed to run a simulation on their own and pass it to entryPoint, so if they mess up it's their fault?
